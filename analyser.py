...
class analyser(QObject):
    ...
    def createColDict(self,identities):
        ### create individual colors for each identity:
    ...
    def write_data(self):
        ...
        dset_id = self.grp.create_dataset("id",shape=(0,),dtype=np.float64,maxshape=(None,))
        dset_frame_occurr = self.grp.create_dataset("frame_occurr",shape=(0,),dtype=np.int64,maxshape=(None,))
        ...
        x,y,ID,frame_occurr = zip(*self.point)

        for i in range(0,len(np.asarray(x)),1):
            self.filename_list.append(str((os.path.splitext(os.path.basename(self.video))[0])))

        dset_list = [dset_x,dset_y,dset_id,dset_frame_occurr,dset_framenr,dset_filename]
        data_list = [np.asarray(x),np.asarray(y),np.asarray(ID),np.asarray(frame_occurr),np.asarray(self.framenr),np.asarray(self.filename_list)]

        for element in zip(dset_list,data_list):
            newsize = element[0].resize(element[1].shape)
            element[0][0:element[1].shape[0]] = element[1]

    def occurrence(self,x): ### occurence for every frame
        occurr = []
        for element in range(0,self.frame_count):
            occurr.append(np.bincount(x)[element]) if (element == e for e in np.unique(x)) else occurr.append(0)
        print(np.bincount(x))
        return(occurr)
        ...
        while(1):
            ...
            if ret and vout.isOpened():
                ...
                if self.wait > 0:
                    ...
                    vout.release()
                    # plt.plot(self.framenr,self.inds)
                    # plt.show()
                    break
                    ...
                    # gray= cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,15,3)
                    # gray= cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,135,18)
                    gray= cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,25,5)
                    ...
                    ingroup = contours[([5 < cv2.contourArea(c)<220 for c in contours])] ### Limit maximum detected contour size
                    ...
                    for element in self.current:
                        if len(self.pre) > 0 and self.frame_count != 0:
                            ...
                            for previous_element in self.pre:
                                ...
                                if dist_euclidean < 15: ### Check distance to objects in previous frame
                                    ...
                                    for i in range(len(mem_i)):
                                        ...
                                        if dist_euclidean < 30 and reassigned == 0: ### Check distance to object in memory
                    ...
                    #             # mean_e = (np.mean(self.neighbors[0]),np.mean(self.neighbors[:][1]))
                    ...
                    # self.inds.append(len(self.current))
                    print("\n",len(self.pre))
...
class reader(QObject):
    ...
    def cumulative_plot(self,data):
        ...
        plt.figure(1)
        for i,element in enumerate(data):
            values, base = np.histogram(data[i][1], bins=1400)
